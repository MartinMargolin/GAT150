...
namespace smile
{
	void Scene::Update(float dt)
	{
		...
		std::for_each(actors.begin(), actors.end(), [dt](auto& actor) mutable { actor->Update(dt); });
		...
		for (size_t i = 0; i < actors.size(); i++)
		{
			for (size_t j = i + 1; j < actors.size(); j++)
			{
				...
				smile::Vector2 dir = actors[i]->transform.position - actors[j]->transform.position;
	...
	{
		std::for_each(actors.begin(), actors.end(), [renderer](auto& actor) mutable { actor->Draw(renderer); });
